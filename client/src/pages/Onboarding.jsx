...
export default function Onboarding() {
  ...
  const complete = async (selectedLevel) => {
    ...
    try {
      ...
      toast.success("Setup complete! Let's learn.");
      navigate("/app");
    } catch (err) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-brand-600 via-brand-700 to-violet-800 p-4 py-10">
      <div className="w-full max-w-lg rounded-2xl bg-white p-8 shadow-2xl dark:bg-slate-800">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Set up your learning</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Tell us about your languages so we can tailor everything to you.
        </p>

        <div className="mt-6 space-y-5">
          <div>
            <label className="label flex items-center gap-2"><Languages className="h-4 w-4" /> Native language</label>
            <select className="input" value={nativeLanguage} onChange={(e) => setNativeLanguage(e.target.value)}>
              {LANGUAGES.map((l) => <option key={l}>{l}</option>)}
            </select>
          </div>
          <div>
            <label className="label flex items-center gap-2"><BookOpen className="h-4 w-4" /> Language you are learning</label>
          ...
          <div>
            <label className="label flex items-center gap-2"><Gauge className="h-4 w-4" /> Your current level</label>
        ...
        <div className="mt-6 space-y-3">
          <button onClick={() => level && complete(level)} disabled={!level || loading} className="btn-primary w-full">
            {loading ? <Spinner size={16} /> : <><span>Start learning</span><ArrowRight className="h-4 w-4" /></>}
          </button>

          <div className="relative py-2 text-center">
            <span className="relative z-10 bg-white px-3 text-xs font-medium text-slate-400 dark:bg-slate-800">or</span>
          ...
          >
            <ClipboardCheck className="h-4 w-4" /> Take placement test (recommended)
          </button>
        </div>

        <div className="mt-6 space-y-4 rounded-xl bg-slate-100 p-4 dark:bg-slate-700/50">
          {step(<Languages className="h-5 w-5" />, "Your languages", "Set your native and target language")}
          {step(<Gauge className="h-5 w-5" />, "Your level", "Pick a level or take the placement test")}
          {step(<ClipboardCheck className="h-5 w-5" />, "Personalization", "AI targets your specific mistakes")}
        </div>
      </div>
    </div>
  );
